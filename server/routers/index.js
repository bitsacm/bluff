...
*/
router.get('/redirect', middleware.generateAccessToken, async (req, res) => {  // Middleware for converting Auth Code into Access Token
...
router.get('/logout/all', middleware.verifyToken, async( req, res) => { 
  try{
    const user = await User.findOne({email : req.cookies.email})
    user.tokens = []
    await res.clearCookie("jwtToken")  
